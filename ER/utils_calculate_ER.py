...
class DFGConstructor:
    ...
    # def create_dfg_from_training(self, training_log: Union[pd.DataFrame, List]) -> Dict[Tuple[str, str], int]:
    #     """
    #     Create DFG from training log with artificial start/end nodes.
    #
    #     Parameters:
    #     - training_log: Training event log (DataFrame or pm4py log object)
    #
    ...
    #     - Directly-follows graph as dictionary
    #     """
    #     print("Manually creating DFG from training log with artificial start/end activities")
    #
    #     # Convert DataFrame to list of traces if needed
    #     if isinstance(training_log, pd.DataFrame):
    #         traces = []
    #         for case_id, case_events in training_log.groupby('case:concept:name'):
    #             trace = []
    ...
    #         training_log = traces
    #
    #     # Initialize DFG dictionary
    #     dfg = {}
    #
    #     # Extract directly-follows relations from traces
    #     for trace in training_log:
    #         # Skip empty traces
    #         if not trace:
    #             continue
    #
    #         # Add start activity to first event
    #         if len(trace) > 0:
    #             start_activity = '▶'
    #             first_activity = trace[0]['concept:name']
    #             start_pair = (start_activity, first_activity)
    #             dfg[start_pair] = dfg.get(start_pair, 0) + 1
    #
    #         # Process directly-follows relations within the trace
    #         for i in range(len(trace) - 1):
    #             current_activity = trace[i]['concept:name']
    #             next_activity = trace[i + 1]['concept:name']
    #             pair = (current_activity, next_activity)
    #             dfg[pair] = dfg.get(pair, 0) + 1
    #
    #         # Add end activity after last event
    #         if len(trace) > 0:
    #             last_activity = trace[-1]['concept:name']
    #             end_activity = '■'
    #             end_pair = (last_activity, end_activity)
    #             dfg[end_pair] = dfg.get(end_pair, 0) + 1
    #
    #     return dfg
    ...
    def create_dfgs_from_rolling_training(self, seq_test_log: Dict[str, pd.DataFrame],
        ...
        # # Manually create the DFG for the entire training log
        ...
        # dfg_json = self.create_dfg_from_truth(horizon_unit_training_dfg)
    ...
    def create_dfgs_from_rolling_window(self, seq_test_log: Dict[str, pd.DataFrame]) -> Dict[str, Dict]:
        ...
        for window_key, sublog in seq_test_log.items():
            ...
            # # Manually create directly-follows graph instead of using pm4py.discover_dfg
            # dfg = self.create_dfg_from_training(sublog)
...
class ERCalculator:
    ...
    def convert_dfg_into_automaton_without_end_counts(self, nodes: List[Dict], arcs: List[Dict]) -> Tuple[
        Dict, List, Dict, Dict]:
        """
        Convert DFG to automaton ignoring '■' (end) transitions when calculating probabilities
        """
        agg_outgoing_frequency = {}
        node_info = {node['id']: node['label'] for node in nodes}
        ...
        # First pass: calculate outgoing frequencies excluding transitions to '■'
        for arc in arcs:
            if arc['freq'] > 0:
                target_label = node_info.get(arc['to'])
                # Only count this arc in outgoing frequency if target is NOT the end symbol
                if target_label != '■':
                    arc_from = agg_outgoing_frequency.get(arc['from'], 0)
                    agg_outgoing_frequency[arc['from']] = arc_from + arc['freq']
        ...
        for arc in arcs:
            if arc['freq'] > 0:
                if arc['to'] not in sinks:
                    ...
                    # Special handling for start/end symbols
                    if node_info[from_node] == '▶' or label == '■':
                        # Always use probability 1.0 for start/end related transitions
                        transitions[(from_node, label)] = (to_node, 1.0)
                    else:
                        # Use modified outgoing probability that excludes transitions to end symbol
                        transitions[(from_node, label)] = (to_node, arc['freq'] / agg_outgoing_frequency[from_node])
        ...
        for (t_from, label), (t_to, a_prob) in transitions.items():
            ...
            states.add(t_from)
            states.add(t_to)
            t_f = outgoing_prob.get(t_from, 0)
            outgoing_prob[t_from] = t_f + a_prob

        final_states = {}
        for state in states:
            if state not in outgoing_prob or 1.0 - outgoing_prob[state] > 0.000006:
                d_p = outgoing_prob.get(state, 0)
                final_states[state] = math.log(1 - d_p)
    ...
    def calculate_entropic_relevance(self, dfg: Dict, log_truth: Union[pd.DataFrame, List]) -> Tuple[float, int, int, Dict, Dict, Dict]:
        ...
        """
        transitions, sources, final_states, trans_table = self.convert_dfg_into_automaton_without_end_counts(
            dfg['nodes'], dfg['arcs']
        )
