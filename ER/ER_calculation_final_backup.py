...
seq_test_log = extract_rolling_window_sublogs(log, 'case:concept:name', 'concept:name', 'time:timestamp', start_time, horizon)
...
# Ensure your predictions dataframe has the window start dates matching the seq_test_log keys
window_predictions = {}
...
for window_key in seq_test_log.keys():
    ...
    if not window_pred.empty:
        window_predictions[window_key] = window_pred

# Generate DFGs for all prediction windows
...
# Calculate entropic relevance for all time windows
...
# Calculate metrics for rolling entropic relevance
...
# Also output window-by-window comparison
print("\n===== WINDOW-BY-WINDOW COMPARISON =====")
print(f"{'Window':20} {'Truth ER':10} {'Pred ER':10} {'Abs Error':10} {'% Error':10}")
print("-" * 60)
...
for window_key, metrics in rolling_er_results.items():
    ...
    if not (math.isnan(truth_er) or math.isnan(pred_er)):
        ...
        print(f"{window_key:20} {truth_er:<10.4f} {pred_er:<10.4f} {abs_error:<10.4f} {pct_error:<10.2f}%")
    else:
        print(f"{window_key:20} {truth_er if not math.isnan(truth_er) else 'N/A':<10} "
              f"{pred_er if not math.isnan(pred_er) else 'N/A':<10} N/A N/A")
...
with open(f'{dataset}_{horizon}_{model_group}_{model_name}_er_metrics.json', 'w') as f:
