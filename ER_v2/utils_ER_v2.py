...
class BackgroundModel:
    ...
    def close_trace(self, trace_length, fitting):
...
class ERCalculator:
    ...
    def convert_dfg_into_automaton_without_end_counts(self, nodes: List[Dict], arcs: List[Dict]) -> Tuple[
        ...
        # First, filter out arcs going to '■' for frequency calculation
        # filtered_arcs = [arc for arc in arcs if arc['freq'] > 0 and node_info.get(arc['to']) != '■']
        ...
        return transitions, sources, trans_table
    ...
    def calculate_entropic_relevance(self, dfg: Dict, log_truth: Union[pd.DataFrame, List]) -> Tuple[float, int, int, Dict, Dict, Dict]:
        ...
        """
        transitions, sources, trans_table = self.convert_dfg_into_automaton_without_end_counts(
            dfg['nodes'], dfg['arcs']
        )
        ...
        for source in sources:
            ...
            for t, trace in enumerate(log_truth):
                ...
                if not non_fitting and label == '■':
                    info_gatherer.close_trace(len_trace, True)
                    fitting_traces[trace_pattern] = fitting_traces.get(trace_pattern, 0) + 1
                else:
                    info_gatherer.close_trace(len_trace, False)
