...
def main(args):
    ...
    def objective(trial):
        ...
        wandb.init(
            project="PMF_benchmark_LSTM-interpolate-7d",
            entity="yongboyu",
            config=config,
            group="LSTM-optimization"
        )
    ...
    study = optuna.create_study(
        direction="minimize",
        study_name="LSTM-optimization-interpolate-7d",
    ...
    forecast.to_csv(f'../output/result_interpolate_7d/{dataset}/LSTM_predictions.csv')

    pdf_path = f'../output/result_interpolate_7d/{dataset}/LSTM_predictions.pdf'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train LSTM (Demo).")
    parser.add_argument(
