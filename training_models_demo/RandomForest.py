...
def main(args):
    ...
    def objective(trial):
        ...
        wandb.init(
            project="PMF_benchmark_RandomForest-interpolate-6d",
            entity="yongboyu",
            config=config,
            group="random-forest-optimization"
        )
    ...
    wandb_kwargs = {
        "project": "PMF_benchmark_RandomForest-interpolate-7d",
        "entity": "yongboyu",
        "group": 'random-forest-optimization'
    }
    ...
    study = optuna.create_study(
        direction="minimize",
        study_name="random-forest-interpolate-7d",
    ...
    forecast.to_csv(f'../output/result_interpolate_7d/{dataset}/RandomForest_predictions.csv')
    # forecast.to_hdf(f'result/{dataset}_predictions.h5', key='all_predictions', mode='a')

    pdf_path = f'../output/result_interpolate_7d/{dataset}/RandomForest_predictions.pdf'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Random Forest (Demo).")
    parser.add_argument(
