...
def main(args):
    ...
    def objective(trial):
        ...
        wandb.init(
            project="PMF_benchmark_GRU-interpolate-7d",
            entity="yongboyu",
            config=config,
            group="GRU-optimization"
        )
        ...
        model = RNNModel(
            ...
            training_length=input_chunk_length + 1,
    ...
    study = optuna.create_study(
        direction="minimize",
        study_name="GRU-optimization-interpolate-7d",
    ...
    final_model = RNNModel(
        ...
        training_length=best_params["input_chunk_length"] + 1,
    ...
    forecast.to_csv(f'../output/result_interpolate_7d/{dataset}/GRU_predictions.csv')

    pdf_path = f'../output/result_interpolate_7d/{dataset}/GRU_predictions.pdf'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train GRU (Demo).")
    parser.add_argument(
